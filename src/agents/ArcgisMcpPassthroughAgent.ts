...
function createArcgisMcpTools(baseUrl?: string) {
  return [
    tool(
      async ({ keyword }) => formatLayerMatches(await searchArcgisMcpLayers(keyword, baseUrl)),
      {
        name: "search_layers",
        description:
          "Search the localhost ArcGIS MCP server for feature layers matching a keyword. Returns REST service URLs for matching layers.",
        schema: z.object({
    ...
    tool(
        ...
        formatContentMatches(await searchArcgisMcpContent(keyword, item_type, baseUrl)),
      {
        name: "search_content",
        description:
          "Search the localhost ArcGIS MCP server for ArcGIS content items by keyword and optional item type.",
        schema: z.object({
    ...
    tool(
      ...
      },
      {
        ...
        description:
          "Fetch the first rows of a feature layer attribute table from the localhost ArcGIS MCP server and return them as CSV.",
        schema: z.object({
    ...
    tool(
        ...
        formatFieldSummary(await summarizeArcgisMcpField(service_url, field_name, baseUrl)),
      {
        ...
        description:
          "Summarize a field from a feature layer using the localhost ArcGIS MCP server.",
        schema: z.object({
          service_url: z.string().url().describe("Feature layer REST service URL returned by search_layers."),
...
export function registerArcgisMcpPassthroughAgent(
  ...
  const createGraph = () => {
    const tools = createArcgisMcpTools(ctx.baseUrl);
    ...
    async function agentNode(agentState: any) {
      ...
      const response = await invokeToolPrompt({
        promptText:
          "You are a thin connector to a localhost ArcGIS MCP service for external ArcGIS searches. Use the available tools instead of inventing results. Use this agent only when the user wants layers, services, or ArcGIS items from the local MCP server. Do not treat questions about the active map, this map, this webmap, the currently loaded web map, or layers already visible in the app as MCP tasks; those belong to the built-in map exploration capabilities. Prefer search_layers when the user needs feature layer URLs from the MCP server, search_content for ArcGIS items like dashboards and web maps on the MCP server, get_feature_table to inspect fields and sample rows, and summarize_field for statistics about a specific field. After tools run, provide a concise answer grounded only in the returned tool output.",
        messages,
        tools,
        temperature: 0,
      });
      ...
      const toolMessages = await Promise.all(
        lastAiMessage.tool_calls.map(async (toolCall) => {
          ...
          try {
            ...
            return new ToolMessage({
              content: error?.message || "Failed to reach the localhost ArcGIS MCP service.",
              tool_call_id: toolCall.id ?? toolCall.name,
              status: "error",
            });
          }
    ...
    function respondNode(agentState: any) {
      ...
      return {
        outputMessage:
          ...
          "The localhost ArcGIS MCP service did not return a final response.",
      };
    }
  ...
  const agent = {
    id: agentId,
    name: "ArcGIS MCP Search",
    description:
      "Searches external ArcGIS layers and content through the localhost MCP server. Use this for MCP-server searches, feature service URLs, feature table inspection, and field summaries from the local server. Do not use this for questions about the active map, this webmap, current map layers, or content already loaded in the app.",
    createGraph,
    workspace: {},
  } as any;
